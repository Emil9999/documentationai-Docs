...
| `status`                 | enum   | The current synchronization status of the application (`SUCCESS`, `SUCCESS_FALLBACK`, `FAILURE`, `EXPECTED_FAILURE`, `NOTSENT`). |
...
- **EXPECTED_FAILURE**: Synchronization failed due to an expected error, such as `DuplicateApplicationError` (the candidate already exists in the ATS) or `JobNotPublishedError`. No action is needed — see [Duplicate Applications](/guides/duplicate-applications).
