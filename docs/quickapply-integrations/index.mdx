...
```
https://api.getkini.com/webhooks/{partner}/{kini_agency_id}/{kini_company_id}/{external_job_id}
```

- **partner**: the integration slug (`indeed`, `monster`, `jobrapido`, `heyflow`, `meta`, `perspective`).
- **kini_agency_id**: the agency ID provided by the Kini team.
- **kini_company_id**: the Kini company the application should be forwarded to.
- **external_job_id**: the job's ID in the connected ATS.

See each partner page for the exact URL, an example, and any partner-specific fields.
