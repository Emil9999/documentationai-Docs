...
The slugs you pass as `ats` and `integration`, and the fields `compose` expects, come from the [Integrations](/api/integrations/index) endpoints.
...
- Send a company admin a magic link so they can complete their own integration setup in Kini's UI, without being issued an API key.
...
- `POST /companies/{id}/compose/` overwrites any existing configuration for the same company and integration, so the call is safe to repeat.
