---
title: "Filtering"
description: "Efficient data retrieval often requires filtering the dataset to match specific criteria. The Kini API supports filtering across multiple endpoints."
---

<Callout kind="info">
  ### Supported Endpoints

Filtering is supported for the **GET /jobs**, **GET /applications**, and **GET /companies** endpoints. Ensure you use the correct parameters to avoid unnecessary data processing.
...
| Parameter                | Description                                                                                                  | Filter Type           | Example Usage                               |
| :----------------------- | :------------------------------------------------------------------------------------------------------------| :--------------------- | :------------------------------------------ |
| `email`                  | Filters applications by applicant email                                                                     | Exact match           | `/applications?email=applicant@example.com` |
| `partner_application_id` | Filters by the partner's application ID                                                                     | Exact match           | `/applications?partner_application_id=8262` |
| `partner_job_id`         | Filters by the partner's job ID                                                                             | Exact match           | `/applications?partner_job_id=123_4`        |
| `partner_company_id`     | Filters by the partner's company ID                                                                         | Exact match           | `/applications?partner_company_id=acme-inc` |
...
| `sync_status`            | Filters by the synchronization status. Possible values: `SUCCESS`, `SUCCESS_FALLBACK`, `EXPECTED_FAILURE`, `FAILURE`, `NOTSENT` | Exact match           | `/applications?sync_status=SUCCESS`         |
...
| Parameter            | Description                                                                                                                | Filter Type                 | Example Usage                                  |
| :------------------- | :-------------------------------------------------------------------------------------------------------------------------| :--------------------------- | :---------------------------------------------- |
| `name`               | Filters companies by their name                                                                                            | Contains (case-insensitive) | `/companies?name=Example`                      |
| `ats`                | Filters companies by their applicant tracking system (ATS)                                                                 | Exact match                 | `/companies?ats=personio`                      |
| `partner_company_id` | Filters companies by their partner's internal reference (if provided)                                                      | Exact match                 | `/companies?partner_company_id=EXT-COMPANY-001` |
| `status`             | Filters companies by lifecycle status. Possible values: `invited`, `requested`, `active`, `failing`, `deactivated`, `deleted`. Repeat the parameter to match several values | Exact match                 | `/companies?status=active&status=failing`      |
| `search`             | Searches across `name`, the ATS slug, and `partner_company_id`                                                             | Contains (case-insensitive) | `/companies?search=Example`                    |

<Callout kind="info">
  `job_sync_active` and `candidate_sync_active` are returned in the response but cannot be
  filtered on — they are derived from the company's entitlements rather than stored on the
  record.
</Callout>
...
1. **Filter Jobs by Status and City**\
   Retrieve all active jobs located in Berlin:
   ```
   GET /jobs?status=active&city=Berlin
   ```
2. **Filter Applications by Creation Date**\
   Retrieve applications created after January 1, 2024:
   ```text
   GET /applications?created_at=2024-01-01
   ```
...
{
  "count": 125,
  "next": "https://api.getkini.com/jobs?status=active&page=2&page_size=20",
  "previous": null,
  "results": [
    {
      "id": 42,
...
1. **Combine Filtering with Pagination**\
   ...
   ```text
   GET /jobs?status=active&city=Berlin&page=1&page_size=20
   ```
