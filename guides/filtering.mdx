...
Filtering is supported for the **GET /jobs** and **GET /applications** endpoints. Ensure you use the correct parameters to avoid unnecessary data processing.
...
| `partner_company_id`    | Filters companies by their partner's internal reference (if provided)  | Exact match                 | `/companies?partner_company_id=123`     |
...
1. **Combine Filtering with Pagination**\
   ...
   ```text
   GET /jobs?status=active&city=Berlin&page=1&limit=20
   ```
