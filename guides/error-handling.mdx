---
title: Error Handling
description: >-
  The Kini API is designed to provide clear and actionable error messages to
  help you diagnose and resolve issues quickly.
---
## Common Error Codes

### 1. Authentication Errors

| Status Code | Error Message | Resolution                                                                                                                                                          |
| ----------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `401`       | Unauthorized  | Ensure the `Authorization` header contains a valid token. Rotate the API key in the [Kini Partner App](https://app.getkini.com/) if it has expired or been revoked. |
| `403`       | Forbidden     | Verify the user's permissions for the requested endpoint.                                                                                                          |

<br />

### 2. Validation Errors

| Status Code | Error Message          | Description                                         | Resolution                                                          |
| ----------- | ----------------------- | ---------------------------------------------------- | -------------------------------------------------------------------- |
| `400`       | Invalid Input          | Request body or query parameters are malformed.     | Validate the request payload against the API specification.         |
| `400`       | Missing Required Field | A required field is missing in the request payload. | Include all mandatory fields as specified in the API documentation. |

<br />

### 3. Rate Limiting

| Status Code | Error Message     | Description                                         | Resolution                                                                                |
| ----------- | ------------------ | ----------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `429`       | Too Many Requests | Exceeded the allowed number of requests per minute. | Implement retry logic with exponential backoff. Monitor usage to stay within rate limits. |

<br />

### 4. Server Errors

| Status Code | Error Message         | Description                                   | Resolution                                                                            |
| ----------- | ----------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| `500`       | Internal Server Error | Unexpected error occurred on the server.      | Retry the request after a short delay. If the issue persists, please contact support. |
| `503`       | Service Unavailable   | The server is temporarily overloaded or down. | Wait and retry after a few minutes. If the issue persists, please contact support.    |

---

## Error Response Format

The Kini API uses Django REST Framework's standard error format. The shape depends on the error type:

**Field validation errors (400)** return a JSON object keyed by field name, with a list of error messages for that field:

```json
{
    "title": [
        "This field is required."
    ]
}
```

**All other errors** (authentication, permissions, not found, rate limiting) return a JSON object with a single `detail` field:

```json
{
    "detail": "Authentication credentials were not provided."
}
```

### Fields

| Field          | Applies to                     | Description                                                |
| :------------- | :------------------------------ | :----------------------------------------------------------- |
| `<field_name>` | Field validation errors (400)  | Array of error messages for that specific request field.   |
| `detail`       | All other errors               | Short, human-readable description of the error.             |

---

## Best Practices for Error Handling

1. **Log Errors:**\
   Log error responses in your application for debugging and monitoring.
2. **Implement Retry Logic:**\
   For 429 and 5xx errors, implement retry logic with exponential backoff.
3. **Validate Requests:**\
   Ensure all required fields are included and conform to the API schema before making a request.
4. **Handle Token Expiry:**\
   Monitor for 401 Unauthorized responses and rotate the API key from the [Kini Partner App](https://app.getkini.com/).
5. **Monitor API Limits:**\
   Keep track of your API usage to avoid rate limiting errors (429 Too Many Requests).

---

## Contact Support

If you encounter persistent errors or need assistance resolving an issue, please send us an Email at: [tech@getkini.com](mailto:tech@getkini.com)
